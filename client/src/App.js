...
function App() {
    const [memberId, setMemberId] = (0, react_1.useState)("member-1");
    const [type, setType] = (0, react_1.useState)("flu_shot");
    ...
    async function createAction() {
        ...
        setMessage(null);
        try {
            const res = await fetch(`${API_BASE_URL}/members/${encodeURIComponent(memberId)}/actions`, {
                method: "POST",
                headers: {
    ...
    return (<div style={{
      ...
      <div style={{ marginTop: "1rem" }}>
        <label style={{ display: "block", marginBottom: 8 }}>
          Member ID:
          <input value={memberId} onChange={(e) => setMemberId(e.target.value)} style={{
            display: "block",
            width: "100%",
            padding: "0.4rem",
            marginTop: 4,
        ...
        <label style={{ display: "block", marginBottom: 8 }}>
          Action Type:
          <input value={type} onChange={(e) => setType(e.target.value)} placeholder="e.g. flu_shot" style={{
            display: "block",
            width: "100%",
            padding: "0.4rem",
            marginTop: 4,
      ...
      {message && (<div style={{ marginTop: "1rem", color: "#1a5" }}>{message}</div>)}

      <h2 style={{ marginTop: "1.5rem" }}>Actions</h2>
      {actions.length === 0 ? (<p style={{ color: "#777" }}>No actions loaded yet.</p>) : (<ul>
